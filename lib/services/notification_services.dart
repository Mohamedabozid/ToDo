...
import '../ui/pages/notification_screen/notification_screen.dart';
...
class NotifyHelper {
  ...
  initializeNotification() async {
    ...
    const AndroidInitializationSettings initializationSettingsAndroid =
        AndroidInitializationSettings('ic_stat_done');
  ...
  tz.TZDateTime _nextInstanceOfTenAM(
    ...
    var foramtedDate = DateFormat.yMd().parse(date!);
    // final tz.TZDateTime fd = tz.TZDateTime.from(foramtedDate, tz.local);
    print('now $now');
    // tz.TZDateTime scheduledDate =
    ...
    if (scheduledDate.isBefore(now)) {
      if (repeat == 'Daily') {
        scheduledDate = tz.TZDateTime(tz.local, now.year, now.month,
            (foramtedDate.day) + 1, now.hour, now.minute);
      }
      if (repeat == 'Weekly') {
        scheduledDate = tz.TZDateTime(tz.local, now.year, now.month,
            (foramtedDate.day) + 7, now.hour, now.minute);
      }
      if (repeat == 'Monthly') {
        scheduledDate = tz.TZDateTime(tz.local, now.year,
            (foramtedDate.month) + 1, foramtedDate.day, now.hour, now.minute);
      }
